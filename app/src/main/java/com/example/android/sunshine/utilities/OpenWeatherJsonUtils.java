...
//        long normalizedUtcStartDay = SunshineDateUtils.normalizeDate(now);
        ...
        for (int i = 0; i < jsonWeatherArray.length(); i++) {
            ...
            JSONObject mainJson = dayForecast.getJSONObject(OWM_MAIN);
            JSONObject windJson = dayForecast.getJSONObject(OWM_WIND);

            pressure = mainJson.getDouble(OWM_PRESSURE);
            humidity = mainJson.getInt(OWM_HUMIDITY);
            high = mainJson.getDouble(OWM_MAX);
            low = mainJson.getDouble(OWM_MIN);

            windSpeed = windJson.getDouble(OWM_WINDSPEED);
            windDirection = windJson.getDouble(OWM_WIND_DIRECTION);
